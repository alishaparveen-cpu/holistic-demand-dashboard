...
WITH ld AS (SELECT DISTINCT RIGHT(phone_no,10) AS ph, created_at AS ld_ts FROM allo_persons.lead
  WHERE phone_no IS NOT NULL AND LEN(phone_no)>=10 AND created_at>='2026-05-04' AND created_at< '2026-06-15'),
oc AS (SELECT RIGHT("to",10) AS ph, start_time AS ct FROM allo_vendors.exotel_calls
  WHERE direction IN ('outbound','outbound-api') AND deleted_at IS NULL AND start_time>='2026-05-04'),
