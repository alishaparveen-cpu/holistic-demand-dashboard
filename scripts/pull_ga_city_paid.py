...
import datetime as _dt   # DYNAMIC week grid: 54 Mondays newest-first, ending at the latest COMPLETE Mon–Sun week (zero-touch; was hardcoded → went stale)
_lm = _dt.date.today() - _dt.timedelta(days=_dt.date.today().weekday()) - _dt.timedelta(days=7)
WEEKS = [(_lm - _dt.timedelta(weeks=i)).isoformat() for i in range(54)]   # newest-first, dashboard grid
