...
SELECT
  ...
  -- Schema change 2026-06: a.consultation_type was removed. Offline = booked at a real
  -- physical clinic (location has a city); online/tele = no clinic location or 'Practo Online'.
  CASE WHEN loc.city IS NOT NULL AND loc.city <> '' AND loc.city <> 'Practo Online'
       THEN '1' ELSE '0' END                   AS offline_location_flag
FROM allo_consultations.appointments a
-- Standard booking unit = Screening Call (replaces the old consultation_type filter that
-- excluded lab/home/admin slots). Matches the rest of the dashboard's booking definition.
JOIN allo_consultations.types typ
  ON typ.id = a.type_id AND typ.name = 'Screening Call'
...
LEFT JOIN allo_persons.providers pr
  ON pr.id = a.provider_id AND pr.deleted_at IS NULL
LEFT JOIN diag d
...
def main():
    # SSO profile locally; default/env IAM credentials in CI (matches scripts/redshift_query.py)
