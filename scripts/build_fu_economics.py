#!/usr/bin/env python3
"""Build data_fu_econ.json — DONE-DATE economics (L2 D2P, invoiced), per clinic × week.
...
appt_level AS (
  SELECT ap.id AS ap_id,
    ...
    CASE WHEN typ.name='Follow Up' AND (CASE WHEN aploc.id IN ({TELE}) OR aploc.id IS NULL THEN 0 ELSE 1 END)=1 THEN 'fu_offline'
...
SELECT doc_city, doc_locality, doctor, week_start, diagnosis,
  COUNT(CASE WHEN segment='fu_offline' THEN 1 END) AS done,
  SUM(CASE WHEN segment='fu_offline' AND purchased_flag=1 THEN 1 ELSE 0 END) AS purchased,
  ROUND(SUM(CASE WHEN segment='fu_offline' THEN med_amt  ELSE 0 END)) AS meds_val,
  ROUND(SUM(CASE WHEN segment='fu_offline' THEN test_amt ELSE 0 END)) AS test_val,
  ROUND(SUM(CASE WHEN segment='fu_offline' THEN ther_amt ELSE 0 END)) AS ther_val,
  ROUND(SUM(CASE WHEN segment='fu_offline' THEN cons_amt ELSE 0 END)) AS cons_val
FROM appt_level
WHERE week_start >= DATE_TRUNC('week', DATEADD(month, -{MONTHS_BACK}, CURRENT_DATE))::date AND segment='fu_offline'
...
def main():
    ...
    out = {"_meta": {"weeks": weeks,
                     "source": "L2 D2P (invoiced) · DONE-date offline FOLLOW-UP · ops revenue",
                     ...
                     "fields": FIELDS},
           "clinics": clinics}
    json.dump(out, open(os.path.join(ROOT, "data_fu_econ.json"), "w"), separators=(",", ":"))

    vwk = "2026-06-22"
    def cs(city, f):
        return sum(o[f][widx[vwk]] for k, o in clinics.items() if k.split("|")[0] == city) if vwk in widx else 0
    print(f"data_fu_econ.json · {len(clinics)} clinics · {NW} weeks ({weeks[0]}→{weeks[-1]})")
