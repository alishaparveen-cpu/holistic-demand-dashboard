...
def main():
    ...
    for line in p.stdout.splitlines():
        ...
        if len(c) < 17:
            continue
        city, clinic, wk, pt, la, ch, md, num, cmp, cat, catsrc, diag, br, dr, doctor, n, dn = c   # cat = 3-tier waterfall (campaign→call→diag); catsrc = how it was decided; diag = done clinical dx; doctor = provider of the week's first SC (episode grain, offline)
        if wk not in WI:
            continue
        key = f'{city}|{clinic}'
        chs = CH.get(ch, 'Other')
        md2 = md if chs != 'Other' else 'other'   # untaxonomy channels are flat
        cell = acc[key][(pt, la, chs, md2, num, cmp, cat, catsrc, diag, br, dr, doctor)]
        cell[0][WI[wk]] += int(n)
    ...
    for key, cells in acc.items():
        arr = [{'pt': k[0], 'la': k[1], 'ch': k[2], 'md': k[3], 'num': k[4], 'cmp': k[5], 'cat': k[6], 'csrc': k[7], 'dg': k[8], 'br': k[9], 'dr': k[10], 'doc': k[11], 'w': v[0], 'wd': v[1]} for k, v in cells.items()]
