...
def bookings_by_source(cfg):
    ...
    sql = """WITH bk0 AS (
      ...
      WHERE a.deleted_at IS NULL AND a.created_at >= '{lo}' AND a.created_at < '2026-07-13'),
...
def call_lead_book(num_list, paid_num, cfg, kind):
    ...
    sql = """WITH calls AS (
      ...
      WHERE {where} AND {extra} {locf}
        AND (ec.start_time+INTERVAL '5 hours 30 minutes') >= '{lo}' AND (ec.start_time+INTERVAL '5 hours 30 minutes') < '2026-07-13'
      GROUP BY 1,3),
...
def gpaid_web_leadbook(cfg, bkphones):
    ...
    WHERE (gclid<>'' OR LOWER(utm_source)='google')
      ...
      AND created_at>='{lo}' AND created_at<'2026-07-13';""".format(tok=tok, lo=LO)
...
def availability(cfg):
    FROM = ("""FROM allo_consultations.roster_slots rs
      ...
      WHERE rs.type_id='cd02525c-1528-4047-a12c-1ad526c28c9a' AND rs.available_for_booking=1
        AND rs.start_time >= '{lo}' AND rs.start_time < '2026-07-13'
...
def reviews(cfg):
    ...
    WHERE er.deleted_at IS NULL AND LOWER(er.platform) IN ('google','gmb')
      AND er.review_date >= '{lo}' AND er.review_date < '2026-07-13'
