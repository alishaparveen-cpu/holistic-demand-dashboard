...
WITH gl AS (
  SELECT DISTINCT leads.phone_no1 AS ph,
    ...
    COALESCE(leads.call_location,'') AS cl, COALESCE(ldr.utm_campaign,'') AS utm, leads.call_booking_ts AS bk
  ...
  WHERE leads.source='Google' AND leads.created_on_date >= '2026-03-23'
),
dn AS (
...
SELECT gl.wk, gl.cl, gl.utm, COUNT(DISTINCT gl.ph) leads,
...
FROM gl LEFT JOIN dn ON dn.ph=gl.ph GROUP BY 1,2,3;
"""
...
def main():
    ...
    for line in p.stdout.strip().splitlines():
        c = line.split("\t")
        if len(c) < 6: continue
        wk, cl, utm = c[0], c[1], c[2]
        if wk not in idx: continue
        try: lds, bk, dn = int(c[3]), int(c[4]), int(c[5])
        except ValueError: continue
        cy = city_of(cl, utm); i = idx[wk]
        a = city.setdefault(cy, {"leads": [0]*12, "booked": [0]*12, "done": [0]*12})
        a["leads"][i] += lds; a["booked"][i] += bk; a["done"][i] += dn
    out = {"_meta": {"source": "main_source_wise_leads source=Google → city via clinic locality else utm_campaign; booked=call_booking_ts; done=SC COMPLETED; total (web+call), reconciles to network Google leads",
