-- gclid (Google Ads) leads + bookings by utm_campaign × week. Tagged to city/category in the build
-- via the campaign name, so the funnel aligns with the GA clicks pull (data_ga_daily.json).
WITH ga_leads AS (
  SELECT DISTINCT RIGHT(phone_no,10) AS ph, COALESCE(NULLIF(TRIM(utm_campaign),''),'(none)') AS camp,
    DATE(created_at) AS ld_date, TO_CHAR(DATE_TRUNC('week', created_at),'YYYY-MM-DD') AS wk
  FROM allo_persons.lead
  WHERE created_at >= '2026-02-09' AND created_at < '2026-06-08'
