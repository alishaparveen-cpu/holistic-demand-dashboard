...
CHANS = ["gmb","google_ad","whatsapp","direct","organic","fb","justdial","others"]
# fetch_leads.sql column order: city, clinic, wk_mon, google_ad, gmb, whatsapp, direct, organic, fb, justdial, others, total
COLS = ["google_ad","gmb","whatsapp","direct","organic","fb","justdial","others"]
...
for line in p.stdout.strip().splitlines():
    ...
    o = D.setdefault(f"{city}|{clinic}", {ch:[0]*12 for ch in CHANS})
    for j, col in enumerate(COLS):
        try: o[col][idx[wk]] += int(float(c[3+j]))
        except (ValueError, IndexError): pass
out = {"_meta":{"source":"production.public.main_source_wise_leads (Redshift, hourly)",
                "weeks":WEEKS, "fields":"gmb=listing clicks+inbound calls · google_ad=paid · whatsapp · direct · organic · fb=Meta · justdial · others"}}
out.update(D)
