-- Per clinic: scheduled / shrunk(blocked) / available bookable slots by day-of-week × hour (IST),
-- last 8 weeks. Pairs with data_booking_hod.json for an hour-granular availability-loss.
WITH slots AS (
  ...
  WHERE rs.type_id='cd02525c-1528-4047-a12c-1ad526c28c9a'
    AND rs.start_time >= '2026-04-06' AND rs.start_time < '2026-06-01'
    AND COALESCE(loc.locality,loc.name,'')<>'' AND LOWER(COALESCE(loc.locality,loc.name,''))<>'online'
  GROUP BY 1,2,3,4, rs.provider_id, rs.start_time
)
SELECT city||'|'||clinic AS k, dow, hr,
