-- L2C by CITY (last 4 complete weeks): leads -> reached -> connected -> booked.
WITH ld AS (
  SELECT DISTINCT RIGHT(phone_no,10) AS ph, DATE(created_at) AS ld_date, NULLIF(TRIM(city),'') AS city
  FROM allo_persons.lead
  WHERE phone_no IS NOT NULL AND LEN(phone_no)>=10 AND created_at>='2026-05-11' AND created_at< '2026-06-29' AND city IS NOT NULL),
...
bk AS (SELECT DISTINCT RIGHT(p.phone_no,10) AS ph, DATE(a.created_at) bdate FROM allo_consultations.appointments a
  ...
  JOIN allo_persons.patient p ON p.id=a.patient_id WHERE a.deleted_at IS NULL AND a.created_at>='2026-05-11' AND p.phone_no IS NOT NULL)
...
GROUP BY 1 HAVING COUNT(DISTINCT ld.ph)>=50 ORDER BY leads DESC;
