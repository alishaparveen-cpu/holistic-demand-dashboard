...
by_lifecycle=[{'src':r[0],'booked':int(r[1]),'done':int(r[2]),'noshow':int(r[3]),'resched':int(r[4]),'cancelled':int(r[5]),'pending':int(r[6]),'recovered':int(r[7])} for r in q('fetch_lead_lifecycle.sql') if len(r)>=8]
out={'_meta':{'source':'allo_persons.lead + exotel_calls + SC appts','note':'Lead→booking by contact mode (priority inbound>outbound>WhatsApp>website). booked within 14d. by_source/by_lifecycle=4wk. NETWORK-level.','window':(weeks[0]+'..'+weeks[-1]) if weeks else ''},'weeks':weeks,'by_mode':by_mode,'by_source':by_source,'by_lifecycle':by_lifecycle}
