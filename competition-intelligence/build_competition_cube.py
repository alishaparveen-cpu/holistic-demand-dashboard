...
ROOT = os.path.dirname(os.path.abspath(__file__))  # self-contained
...
def dfs_cat(cat, name):
    """GMB category from the crawl by name across ALL keys (category is business-intrinsic). Same cat only."""
    tgt=_n(name)
    if not tgt: return None
    best=None
    for ee in DFS.get(cat,{}).values():
        ...
        for cc in ee.get('competitors',[]):
            ...
            if best is None and (nm[:18]==tgt[:18] or (len(tgt)>6 and (tgt in nm or nm in tgt))): best=cc['category']
    return best
...
def build_cat_grid(cat, rows, cube):
    ...
    for (city, loc), lst in byclinic.items():
        ...
        for r in lst:
            ...
            km = gkm if gkm is not None else dfs_km(cat, key, name)
    ...
    for k, e in DFS.get(cat, {}).items():
        ...
        if not comps_all: continue
        rel = [c for c in comps_all if c['rel'] is True] or [c for c in comps_all if c['rel'] is not False] or comps_all
        # crawl-only clinic: order by the REAL local-pack position (who Google actually ranks), reviews as tiebreak
        rel.sort(key=lambda c: (c.get('pos') or 999, -c['reviews']))
        comps = [rel[0]] + sorted([c for c in comps_all if c is not rel[0] and c['rel'] is not False], key=lambda c: (c.get('pos') or 999, -c['reviews']))[:6]
        top = comps[0]
